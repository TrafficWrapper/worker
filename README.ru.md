...
- `CAMOUFLAGE_DOMAIN`: реальный TLS 1.3 SNI/fallback домен для REALITY, не
  `example.com`.
...
| `CAMOUFLAGE_DOMAIN` | REALITY serverName/camouflage SNI и fallback identity. | Обяз. для REALITY | `example.com` placeholder | Используйте реальный TLS 1.3 домен, например `www.example.org`, подходящий вашему deployment; не оставляйте `example.com` в production. |
