...
## Ротация диалекта AWG

Диалект нельзя сменить на том же порту, не сломав всех клиентов, поэтому он
ротируется через профили: добавьте в `AWG_INBOUNDS` профиль с
`"own_dialect": true` на новом порту и подсети (и свой сервис `awg-gw`),
оркестратор переводит клиентов на него по `awg_profiles` из self-describe,
после чего старый профиль удаляется. `WORKER_DIALECT_WIDE=1` берёт параметры
junk-пакетов (Jc, Jmin, Jmax) из широких диапазонов; включайте только когда
все клиенты их принимают.

`min_version_code` профиля не даёт его слишком старым клиентам. Это код,
выведенный из имени версии приложения: major*10000 + minor*100 + patch
(0.1.31 → 131), а не Android `versionCode`; оркестратор сравнивает его с таким
же кодом каждого клиента.

## Лимиты скорости AWG на устройство

`desired_state.approved_devices[].limits.download_mbps` / `upload_mbps`
записываются в реестр пиров AWG. `awg-gw` перечитывает реестр каждые 30 с и
при изменении лимитов пересобирает `tc` на туннельном интерфейсе: HTB-класс на
клиента для download и ingress-полисер для upload. REALITY-клиенты не
ограничиваются (в Xray нет шейпинга по пользователю).
